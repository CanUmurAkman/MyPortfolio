...
#Using (c) assignment constraints and connectivity constraints
...
for size in range(1, num_nodes):
    for subset in itertools.combinations(range(num_nodes), size):
        complement_subset = set(range(num_nodes)) - set(subset)

        model.addConstr(
            gp.quicksum(x[i, j] for i in subset for j in complement_subset) >= 1,
            f'Connectivity_{subset}'
        )
...
# 
# Total Distance: 14372.358696733374
# 
# part (a) and part (c) gave very different results! part (a) is much better because of giving a lower total distance for the same CPU usage! CPU usages are very similar! part (b) is slightly better than part (a) due to lower total distance and few use of CPs.
